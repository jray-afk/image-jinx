...
"""
Most functions from: https://easings.net/
"""

def ease_in_linear(x):
    return 1 - x


def ease_out_linear(x):
    """
    Might seem trivial, but better to just have function reference for this rather than special logic for linear all
    over functions that require an easing function definition parameter.
    :param x:
    :return:
    """
    return x

def ease_in_out_normal(x):
    """
    Ref: https://stackoverflow.com/questions/13097005/easing-functions-for-bell-curves
    :param x:
    :return:
    """
    return (math.sin(2 * math.pi * (x - 0.25)) + 1) / 2
